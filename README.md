...
Before we can debug a [rebar3](https://rebar3.org/) project with `edb`, we need to ensure that we build the code using the `beam_debug_info`
 option, which adds debug symbol information needed to be able display variable names on a paused process.
